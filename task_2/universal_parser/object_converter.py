...
def refactor_dict(obj):
    ...
    for i in new_obj:
        ...
        elif isinstance(new_obj[i], types.FunctionType) or isinstance(
            new_obj, types.MethodType
        ):
...
def refactor_object(obj):
    ...
    elif isinstance(new_obj, types.FunctionType) or isinstance(
        new_obj, types.MethodType
    ):

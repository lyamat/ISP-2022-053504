...
class TestJson(unittest.TestCase):
    ...
    def test_functions(self):
        self.assertEqual(
            self.parser.loads(self.parser.dumps(function1))(123), function1(123)
        )
        self.assertEqual(
            get_function_code_attributes(
                self.parser.loads(self.parser.dumps(function1))
            ),
            get_function_code_attributes(function1),
        )

        self.assertEqual(
            self.parser.loads(self.parser.dumps(function2))(4, 6), function2(4, 6)
        )
        self.assertEqual(
            get_function_code_attributes(
                self.parser.loads(self.parser.dumps(function2))
            ),
            get_function_code_attributes(function2),
        )

        self.assertEqual(
            self.parser.loads(self.parser.dumps(function3))(2), function3(2)
        )
        self.assertEqual(
            get_function_code_attributes(
                self.parser.loads(self.parser.dumps(function3))
            ),
            get_function_code_attributes(function3),
        )

        self.assertEqual(
            self.parser.loads(self.parser.dumps(function4))("name", "surname"),
            function4("name", "surname"),
        )
        self.assertEqual(
            get_function_code_attributes(
                self.parser.loads(self.parser.dumps(function4))
            ),
    ...
    def test_classes(self):
        ...
        class1_instance = Student(95350103, "Ilya", "Matveyeu", 953505)
        class2_instance = MyTest()

        restored_class1 = self.parser.loads(self.parser.dumps(class1))
        restored_class1_instance = restored_class1(95350103, "Ilya", "Matveyeu", 953505)
    ...
    def test_class_instances(self):
        class_instance1 = Student(95350012, "Kirill", "Basenko", 953505)
        ...
        self.assertEqual(
            class_instance1.student_id, restored_class_instance1.student_id
        )
    ...
    def test_complex_lists_and_dictionaries(self):
        structured_object1 = [
            ...
            True,
            ["1", "2", "3"],
            {"string": ["my_object", None, 3.14, 21232]},
        ]
        structured_object2 = {
            "some_value": True,
            "name": "Ilya",
        ...
        structured_object3 = {
            "scans": [
                {"status": None, "starttime": "20150803T000000", "id": 45},
                {"status": "completed", "starttime": 15, "id": 539},
            ],
            "art": "func",
        }
        ...
        self.assertEqual(
            self.parser.loads(self.parser.dumps(structured_object1)), structured_object1
        )
        self.assertEqual(
            self.parser.loads(self.parser.dumps(structured_object2)), structured_object2
        )
        self.assertEqual(
            self.parser.loads(self.parser.dumps(structured_object3)), structured_object3
        )
        self.assertEqual(
            self.parser.loads(self.parser.dumps(structured_object4))[0](4), 16
        )
        self.assertEqual(
            tuple(self.parser.loads(self.parser.dumps(structured_object4))[2]),
            structured_object4[2],
        )
        self.assertEqual(
            self.parser.loads(self.parser.dumps(structured_object5)).get("some_func")(
                2
            ),
            function1(2),
        )
        restored_structured_object6 = self.parser.loads(
            self.parser.dumps(structured_object6)
        )
        self.assertEqual(
            structured_object6["1"](4), restored_structured_object6["1"](4)
        )
        class_instance1 = structured_object6["2"]["3"]["4"](
            95350103, "Ilya", "Matveyeu", 953505
        )
        restored_class_instance1 = restored_structured_object6["2"]["3"]["4"](
            95350103, "Ilya", "Matveyeu", 953505
        )
        self.assertEqual(
            class_instance1.student_id, restored_class_instance1.student_id
        ...
        self.assertEqual(class_instance1.hello(), restored_class_instance1.hello())
        self.assertEqual(
            structured_object6["5"].const, restored_structured_object6["5"].const
        )
        self.assertEqual(
            structured_object6["5"].hello(), restored_structured_object6["5"].hello()
        )
